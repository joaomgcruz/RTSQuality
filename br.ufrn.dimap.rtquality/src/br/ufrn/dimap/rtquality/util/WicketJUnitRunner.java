...
public class WicketJUnitRunner extends BlockJUnit4ClassRunner {
	 ...
	 */
	private void findMethod(TestClass testClass, List<FrameworkMethod> junitMethods, String methodName, Class<?>... parameterTypes) {
		try {
	...
	private boolean isJUnitMethod(final Method javaMethod) {
		if(Modifier.isPublic(javaMethod.getModifiers()) && (Void.TYPE.equals(javaMethod.getReturnType()) || Void.class.equals(javaMethod.getReturnType())) && javaMethod.getName().startsWith("test"))
			return true;
		for(Annotation annotation : javaMethod.getAnnotations()){
			Annotation teste = javaMethod.getAnnotation(Test.class);
			if(annotation.equals(teste))
				return true;
		}
		return false;
	}
